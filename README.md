# AMINA Malik - Resume

## 👩‍💼 About Me
I am a passionate Front-End Developer focused on building responsive, user-friendly, and visually engaging web applications. I enjoy turning complex ideas into simple, elegant interfaces using modern technologies like HTML, CSS, JavaScript, and frameworks such as React. I’m always eager to learn new tools and improve performance, accessibility, and user experience.

---

## 📌 Contact Information
- 📧 Email: aminamalik0300@gmail.com  
- 📍 Location: Punjab,Pakistan 

---

## 💼 Experience

### **Customers Sales Representative**  
**Nextgen Technologies Inc** | Punjab, Pakistan  
*Jan 2019 – 2021*
- Dedicated Customer Service Representative with experience in handling inbound and outbound calls, resolving customer issues efficiently, and maintaining a high level of customer satisfaction. Skilled in communication, problem-solving, and managing customer relationships in fast-paced call center environments.

---

### **Junior Web developer**  
**Xenova Soft** | Punjab, Pakistan  
*Jan 2024 – Dec 2025*
Assisted in developing and maintaining responsive websites using HTML, CSS, and JavaScript.
Collaborated with senior developers to implement user-friendly UI designs and improve website performance.
Fixed bugs and optimized code to ensure smooth functionality across different browsers and devices.
Worked with version control systems like Git to manage and track project changes.

---

## 🛠️ Skills
- SEO Strategy  
- Content Marketing  
- Web development
- MS Office  
- Copy Editing  
- Project Management  

---

## 🎓 Education

### **Bachelor in Computer Science**  
Fatima Jinnah Women University  
*2024 – 2028*

---

## 📜 Certification
**Certification of Robotics**  
Udemy  
- Gained knowledge of basic robotics concepts, sensors, and automation.

---

## 🌐 Find Me Online
- LinkedIn:  linkedin.com/in/amina-malikkk
 
---

## 📄 Summary
Detail-oriented Junior Web Developer skilled in creating clean and responsive web designs. Experienced in handling customer interactions in fast-paced environments. Focused on delivering quality work and enhancing user satisfaction.

---
